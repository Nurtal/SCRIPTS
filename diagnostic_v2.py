...
for line in data:
	...
	if(cmpt == 0):
		...
		for var in line_in_array:
			if(var == "\\Clinical\\MuscleandSkeletal\\AEARTHRITIS"):
				articulation_index = index
			if(var == "\\Clinical\\Symptom\\ABNORMINFLAM"):
				inflamation_index = index
		...
		if(patient["articulation"] == "NA" or patient["articulation"] == "Unknown"):
			complete_case = False
		patient["inflamation"] = line_in_array[inflamation_index]
...
for patient in cohorte:
	...
	if(patient["inflamation"] == "Present" or patient["inflamation"] == "Past"):
		inflamation = 1
		inflamation += 1
